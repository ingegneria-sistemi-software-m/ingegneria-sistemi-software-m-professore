 ...
 */
    ...
    @JvmStatic
    fun readCellNum(fName: String) : Vector<Int>{
    	val dockerNumSpec = System.getenv("CELLS")
    	if( dockerNumSpec == null ) return readGridConfig(fName)
    	else {
    		val outV = Vector<Int>()
    		RowsNum  = dockerNumSpec.toInt()
    		ColsNum	 = dockerNumSpec.toInt()	
            outV.add( RowsNum )
            outV.add( ColsNum )
            outV.add( 0 )
            CommUtils.outcyan("GridSupport25 | readCellNum in docker RowsNum=$RowsNum, ColsNum=$ColsNum, CellSize=$CellSize")
            return outV
    	}
    }
    ...
    fun readGridConfig(fName: String) : Vector<Int>{
        ...
        outV.add( ColsNum )
        outV.add( CellSize )
        CommUtils.outyellow("GridSupport25 | readGridConfig RowsNum=$RowsNum, ColsNum=$ColsNum, CellSize=$CellSize")
        return outV
    }
    
    @JvmStatic  
    fun emiteventmqttforcells( a:ActorBasic, ev : String ) {
	    for (i in 0 until RowsNum) {
	        for (j in 0 until ColsNum) {	  
	        	val dest = "unibo/qak/cell_${i}_${j}"
	        	CommUtils.outcyan("publish on $dest")
	            a.publish( ev, dest)
	        }
	    }    	
    }
...
//	    CommUtils.outgreen("genCellDescr $outS")
    ...
    @JvmStatic fun createCellsInternal(  a : ActorBasic) {
        ...
        if (cellsList != null) {
            ...
            while (cells.hasNext()) {
                ...
                CommUtils.outblue("GridSupport25 | declareInternalCells Created: $name"  )
            }
        }
    }
    ...
    fun getCellNamesInContext(ctx: String): Struct? {
        ...
        //CommUtils.outcyan("GridSupport25 |  getCellNamesInContext $ctx sol=$sol")
        if (sol.isSuccess) {
    ...
    fun subscribeToNeighborsMqtt( a: ActorBasicFsm, x:Int, y:Int ) : Int{
        var Countnb = 0
        		CommUtils.outcyan("GridSupport25 |  subscribeToNeighborsMqtt $x $y")
 /*1*/ //val nblist=conway.GridSupport25.getCellNeighbors(x,y) //
       //JAN25
	...
	@JvmStatic fun createPlayers(a:ActorBasic, RowsN:Int, ColsN:Int ){
        for( i:Int in 0..(RowsN-1) ) {
        	for( j:Int in 0..(ColsN-1) ) {            
                ...
                //CommUtils.outblue("GridSupport25 | createPlayer Created: $name"  )
            }
        }
    }	
}