...
class Cell0000 ( name: String, scope: CoroutineScope, isconfined: Boolean=false, isdynamic: Boolean=false ) : 
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		//IF actor.withobj !== null val actor.withobj.name» = actor.withobj.method»ENDIF
		  ...
		  /*
		   *---------------------------------------------------------
		  	DICHIARAZIONE DI un outindev per interazione con controller
		    e con al GUI via mqtt
		   *---------------------------------------------------------
		  */
			   
			   lateinit  var outindev  : OutInCellGui  
			   //val guiinterpreter = main.java.CellCmdTranslator( myself )  //MOVED IN PERCEIVER
		 		
		   //NON CAMBIO IL NOME avevdo scelto un nome 'univoco'
		   ...
		   *---------------------------------------------------------
		  	...
		  	TUTTE LE CELLE USANO il NOME DI logger conway per
		  ...
		  */
		   ...
		   */
		  clearlog("./logs/app_conwaycellsqak.log")   
		  
		return { //this:ActionBasciFsm
				state("s0") { //this:State
					action { //it:State
						CommUtils.outblue("$name | INIT ")
						 val eka = CommUtils.ckeckEureka( )  
						CommUtils.outblack("$name | ckeckEureka = $eka")
					...
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("addedtogame(CELLNAME,NROWS,NCOLS)"), Term.createTerm("addedtogame(CELLNAME,NROWS,NCOLS)"), 
								 ...
								 val newname = payloadArg(0)
											...
											//-------------------------------------------------------------
											// Emissione di un log INFO addedcell di category 'cell' 
											// su index cell- (vedi logastash.conf)
											//-------------------------------------------------------------
											   val log = sysUtil.logStr(name, "addedcell $name", "cell") //return jsoobjct
											   CommUtils.outmagenta("log=$log")
								 			   logger.info( log )  
						...
						CommUtils.outblue("$name | STARTS $X,$Y Countnb= $Countnb")
								  ...
								  /*
								   *---------------------------------------------------------
								  	CREAZIONE DI UN outindev per interazione con controller
								    e con al GUI via mqtt
								   *---------------------------------------------------------
								  */
						...
						if(  MyState   
						 ...
						 Countnbmsgs = 0
						           ...
						           displayLedState() 
									...
									//-------------------------------------------------------------
									// Emissione di un log INFO epochEnded di category 'cell' 
									// su index cell- (vedi logastash.conf)
									//-------------------------------------------------------------
									   val log = sysUtil.logStr(name, "epochEnded $name", "cell") //returnn jsoobjct
										CommUtils.outmagenta("log=$log")
						 			   logger.info( log )  
